...
class modBulitabelleHelper {
  ...
  public static function getTabelleAjax()
  {
    $jinput = JFactory::getApplication()->input;
	  ...
	  $context = stream_context_create(array( 
	    'http' => array( 'timeout' => $jparams->get('timeout') )
  	));
	  ...
	  if ($paarungen != FALSE && stristr($paarungen, 'Maximale Abfrageanzahl von 1000 Abfragen pro Tag erreicht!') == FALSE && stristr($paarungen, 'An error has occurred') == FALSE) {
		...
		foreach ($paarungen as $partie) {
		  ...
		  if ($alle_ergebnisse[0] instanceof stdClass) {
		    ...
		    $tabelle[$partie->Team1->TeamName] = array('spiele' => $tabelle[$partie->Team1->TeamName]['spiele'] + 1,
		    ...
		    $tabelle[$partie->Team2->TeamName] = array('spiele' => $tabelle[$partie->Team2->TeamName]['spiele'] + 1,
			  ...
			  $tabelle[$partie->Team1->TeamName] = array('spiele' => 0, 'punkte' => 0, 'tore' => 0, 'gegentore' => 0);
		      $tabelle[$partie->Team2->TeamName] = array('spiele' => 0, 'punkte' => 0, 'tore' => 0, 'gegentore' => 0);
			  if($i == 9) break;
		  }
		}
		...
		foreach($tabelle as $name=>$team) {
		  if ($name == 'SV Sandhausen' && $jparams->get('season') == '2015') $team['punkte'] -= 3;
